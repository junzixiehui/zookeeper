...
public class Leader {
	...
	public LearnerSnapshotThrottler createLearnerSnapshotThrottler(int maxConcurrentSnapshots,
	...
	Leader(QuorumPeer self, LeaderZooKeeperServer zk) throws IOException {
		...
		this.zk = zk;
		this.learnerSnapshotThrottler = createLearnerSnapshotThrottler(maxConcurrentSnapshots,
	...
	void lead() throws IOException, InterruptedException {
		...
		try {
			...
			try {
				...
				shutdown("Waiting for a quorum of followers, only synced with sids: [ " + newLeaderProposal
						.ackSetsToString() + " ]");
			...
			while (true) {
				synchronized (this) {
					...
					if (!tickSkip && !syncedAckSet.hasAllQuorums()) {
						...
						shutdownMessage = "Not sufficient followers synced, only synced with sids: [ " + syncedAckSet
								.ackSetsToString() + " ]";
						break;
					}
					tickSkip = !tickSkip;
	...
	private long getDesignatedLeader(Proposal reconfigProposal, long zxid) {
		//new configuration
		Proposal.QuorumVerifierAcksetPair newQVAcksetPair = reconfigProposal.qvAcksetPairs
				.get(reconfigProposal.qvAcksetPairs.size() - 1);

		//check if I'm in the new configuration with the same quorum address -
		// if so, I'll remain the leader
		if (newQVAcksetPair.getQuorumVerifier().getVotingMembers().containsKey(self.getId()) && newQVAcksetPair
				.getQuorumVerifier().getVotingMembers().get(self.getId()).addr.equals(self.getQuorumAddress())) {
			return self.getId();
		}
	...
	public Proposal propose(Request request) throws XidRolloverException {
		...
		synchronized (this) {
			...
			//leader广播提议 之后会等待收集投票 投票是有follower的 SyncRequestProcessor处理
			sendPacket(pp);
		}
		return p;
	}
			...
			if (ss.getCurrentEpoch() != -1) {
				if (ss.isMoreRecentThan(leaderStateSummary)) {
					throw new IOException(
							"Follower is ahead of the leader, leader summary: " + leaderStateSummary.getCurrentEpoch()
									+ " (current epoch), " + leaderStateSummary.getLastZxid() + " (last zxid)");
				}
				if (isParticipant(id)) {
	...
	private synchronized void startZkServer() {
		...
		LOG.info("Have quorum of supporters, sids: [ " + newLeaderProposal.ackSetsToString()
	...
	public void waitForNewLeaderAck(long sid, long zxid) throws InterruptedException {
		...
		synchronized (newLeaderProposal.qvAcksetPairs) {
			...
			if (zxid != currentZxid) {
				LOG.error("NEWLEADER ACK from sid: " + sid + " is from a different epoch - current 0x" + Long
						.toHexString(currentZxid) + " receieved 0x" + Long.toHexString(zxid));
				return;
			}
